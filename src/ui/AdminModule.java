...
public class AdminModule extends JFrame {
    ...
    private JPanel createManageFacultyPanel() {
        ...
        UIUtils.applyColoredBorder(formCard, " Add New Faculty ");

        JTextField idField   = new JTextField(); UIUtils.styleTextField(idField);
        JTextField nameField = new JTextField(); UIUtils.styleTextField(nameField);
        JTextField userField = new JTextField(); UIUtils.styleTextField(userField);
        JPasswordField passField = new JPasswordField(); UIUtils.styleTextField(passField);
    ...
    private JPanel createMapFacultySubjectsPanel() {
        ...
        try (Connection c = DatabaseConnection.getConnection()) {
            try (Statement s = c.createStatement();
                ...
                while (rs.next()) facultyCombo.addItem(rs.getString("faculty_id"));
            }
            try (Statement s = c.createStatement();
        ...
        assignBtn.addActionListener(e -> {
            ...
            if (fId != null && sFull != null) {
                ...
                try (Connection c = DatabaseConnection.getConnection();
                    ...
                    p.setString(1, fId); p.setString(2, sId);
                    p.executeUpdate();
    ...
    private JPanel createManageStudentsPanel() {
        ...
        JTextField rollField = new JTextField(); UIUtils.styleTextField(rollField);
        JTextField nameField = new JTextField(); UIUtils.styleTextField(nameField);
        JTextField semField  = new JTextField(); UIUtils.styleTextField(semField);
        JTextField deptField = new JTextField(); UIUtils.styleTextField(deptField);
        JPasswordField passField = new JPasswordField(); UIUtils.styleTextField(passField);
        ...
        JButton addBtn    = new JButton("Add");    UIUtils.styleButton(addBtn);
        JButton updateBtn = new JButton("Update"); UIUtils.styleWarnButton(updateBtn);
        JButton delBtn    = new JButton("Delete"); UIUtils.styleDangerButton(delBtn);
        btnRow.add(addBtn); btnRow.add(updateBtn); btnRow.add(delBtn);
        formCard.add(new JLabel());
        formCard.add(btnRow);
        ...
        addBtn.addActionListener(e -> {
            try (Connection c = DatabaseConnection.getConnection();
                ...
                loadStudents(tm); loadDashboardStats();
            } catch (Exception ex) {
                JOptionPane.showMessageDialog(this, "Error: " + ex.getMessage());
            }
        });
        ...
        updateBtn.addActionListener(e -> {
            try (Connection c = DatabaseConnection.getConnection();
                ...
                if (p.executeUpdate() > 0) { JOptionPane.showMessageDialog(this, "Student updated."); loadStudents(tm); }
        ...
        delBtn.addActionListener(e -> {
            ...
            try (Connection c = DatabaseConnection.getConnection();
                ...
                p.setString(1, rollField.getText().trim());
                if (p.executeUpdate() > 0) { JOptionPane.showMessageDialog(this, "Student deleted."); loadStudents(tm); loadDashboardStats(); }
                else JOptionPane.showMessageDialog(this, "Student not found.");
            } catch (Exception ex) { JOptionPane.showMessageDialog(this, "Error: " + ex.getMessage()); }
        });
    ...
    private void addLF(JPanel p, String txt, JTextField f) {
        JLabel l = new JLabel(txt); UIUtils.styleLabel(l); p.add(l); p.add(f);
    }

    private void addLC(JPanel p, String txt, JComboBox<String> c) {
        JLabel l = new JLabel(txt); UIUtils.styleLabel(l); p.add(l); p.add(c);
    }
